...
import edu.washington.multir.util.FigerTypeUtils.FigerAnnotation;
...
public class PersonCountrySententialInstanceGeneration implements
		SententialInstanceGeneration {

	private static PersonCountrySententialInstanceGeneration instance = null;
	
	public static PersonCountrySententialInstanceGeneration getInstance(){
		if(instance == null){
			instance = new PersonCountrySententialInstanceGeneration();
		}
		return instance;
	}
			...
			for(int j =0; j < arguments.size(); j++){
				if(i != j){
					...
					if((arg1 instanceof KBArgument) && (arg2 instanceof KBArgument)){
						...
						if(arg1FigerTypes.contains("/person")  && arg2FigerTypes.contains("/location/country")){
							add = true;
						}
						if(add){
							System.out.println("adding sig " + arg1.getArgName() + " " + arg2.getArgName());
