 ...
 *
 *  Arguments to the main function when invoking class DBPopulateBaselineModelFiles:
 ...
 *       1. Directory containing MILDoc input files (ie "/scratch/MultiR_01/Baseline_Single_MILDocs")
 *       2. Input Manual Annotations file (ie, "/scratch/manualAnnotations.csv")
 *       3. Input Baseline Relations file (ie, "/projects/WebWare6/Multir/MultirSystem/files/targetRelations/partitionRelations.txt")
 *       4. "train" to run Training subset of corpus or "test" to run Test subset.
 *       5. Input file containing list of test documents (ie, "/projects/WebWare6/Multir/MultirSystem/files/testDocuments")
 *       6. Experiment ID (ie, "Baseline_Model")
 *       7. Experiment Date in format YYYY-MM-DD
 *       8. Worker (voter) ID (ie, "Siegfried")
 *       9. Vote Experiment ID (ie, "Third_Attempt")
 *      10. Date of Vote in format YYYY-MM-DD
 *      11. Output Sentences file (ie, "/scratch/MultiR_01/Sentences")
 *      12. Output ExtractionsVotes file (ie, "/scratch/MultiR_01/GenPartExtractionsVotes")
 *      Keyword arg for NER argument identification class (ie, "-ai NERArgumentIdentification")
 *      Keyword arg for Sentential Instance Generation class (ie, "-sig NERSententialInstanceGeneration")
 *      Keyword arg for Feature Generation class (ie, "-fg DefaultFeatureGenerator")
 *
 ...
 *
 */
public class DBPopulateBaselineModelFiles
{
    ...
    public static void main (String[] args) throws ParseException, ClassNotFoundException, InstantiationException, IllegalAccessException, NoSuchMethodException, SecurityException, IllegalArgumentException, InvocationTargetException, SQLException, IOException
    {
        ...
        String pathToTestDocumentsFile = arguments.get(5);
        String experimentID = arguments.get(6);
        String experimentDate = arguments.get(7);
        String workerID = arguments.get(8);
        String voteExperID = arguments.get(9);
        String voteDate = arguments.get(10);
        String pathToSentencesFile = arguments.get(11);
        String pathToExtractionsVotesFile = arguments.get(12);
        ...
        System.out.println("\nPrinting Baseline ExtractionsVotes file ...");
    ...
    private static List<Extraction> getExtractions(Corpus c, ArgumentIdentification ai, SententialInstanceGeneration sig, DocumentExtractor de) throws SQLException, IOException
    {
        ...
        while(docs.hasNext())
        {
            ...
            for(CoreMap sentence : sentences)
            {
                ...
                {
                    Pair<Triple<String, Double, Double>, Map<Integer, Double>> extrResult = de.extractFromSententialInstanceWithFeatureScores(p.first, p.second, sentence, doc);
                    if(extrResult != null)
                    {
